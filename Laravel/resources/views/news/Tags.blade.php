...
@section ('breadcumb')
    ...
    <li class="active">{{$data[0]['Trang Tags Tin tức']}}</li>
@stop()
...
<div class="box-footer">
    <a href="javascript:;" class="btn btn-info" onclick="XoaCheckAll('/admin/news/tags/delete', '{{$data[0]['Thông báo xác nhận xóa']}}')" style="clear: both;">{{$data[0]['Nút Xóa tất cả mục chọn']}}</a>
</div>
@stop()
@section ('footer')
<script>
function InsertData(){
    ...
    $.get('/admin/news/tags/add',{val:val},function(o){
        DirectPage();
    });
}
function UpdateData(id){
    var val = $('.edittagsproduct'+id+' [name="value"]').val();
    $.get('/admin/news/tags/edit',{id:id, val:val},function(o){
        DirectPage();
    });
}
</script>
